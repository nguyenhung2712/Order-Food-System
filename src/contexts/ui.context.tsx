...
function uiReducer(state: State, action: Action) {
  switch (action.type) {
    ...
    case 'SET_FOUNDED_ITEMS': {
      return {
        ...state,
        foundedItems: action.foundedItems,
      };
    }
    case 'SET_ITEMS': {
      return {
...
export const UIProvider: React.FC = (props) => {
  ...
  const setChat = (type: string) =>
    dispatch({ type: 'SET_CHAT', chatType: type });
  const setFoundedItems = (items: any) =>
    dispatch({ type: 'SET_FOUNDED_ITEMS', foundedItems: items });
  const setCurItems = (items: any) =>
