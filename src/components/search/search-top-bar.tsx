...
const SearchTopBar = () => {
  const { openFilter, displayFilter, closeFilter, foundedItems } = useUI();
  ...
  return (
    <div className="flex justify-between items-center mb-6">
      <button
        className="lg:hidden w-[115px] text-skin-base text-sm px-4 py-2 font-semibold border border-skin-base rounded-md flex items-center transition duration-200 ease-in-out focus:outline-none hover:border-skin-primary hover:text-skin-primary"
        onClick={openFilter}
      >
        <FilterIcon />
      ...
      <div className="flex w-full items-center justify-end lg:justify-between">
        <div className="flex-shrink-0 text-skin-base font-medium text-15px leading-4 md:me-6 hidden lg:block mt-0.5">
          {foundedItems.length} {t('text-items-found')}
        </div>
        <ListBox
          options={[
