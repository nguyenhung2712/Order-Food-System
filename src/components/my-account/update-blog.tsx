...
} from '@framework/blog/update-blog';
...
const UpdateBlog: React.FC = () => {
  ...
  const { mutate: updateBlog, isLoading: isUpdateLoading } =
    useUpdateBlogMutation();
  const { data, isLoading: isBlogLoading } = useBlogQuery(slug as string);
  ...
  function onSubmit(input: UpdateBlogInputType) {
    if (isHasContent) {
      ...
      >
        ...
        <div className="relative flex sm:ms-auto mt-5 pb-2 lg:pb-0">
          <Button
            type="button"
            variant="formButton"
            className="w-full sm:w-auto mr-8 bg-transparent text-black hover:text-black hover:bg-skin-primary"
            onClick={() => router.back()}
          >
            Cancel
          </Button>
