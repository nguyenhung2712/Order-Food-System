...
function RenderPopupOrAddToCart({ data }: { data: Product1 }) {
...
const ProductCard: React.FC<ProductProps> = ({ product, className }) => {
  const { dishName, image, type, dishNameEn } = product ?? {};
  ...
  const { price, basePrice } = usePrice({
    amount: Number(
      locale === 'vi' ? product.price : (product.price * 43) / 1000000
    ),
    baseAmount: Number(
      locale === 'vi' ? product.price : (product.price * 43) / 1000000
    ),
  ...
  const priceTemp = product.DishHasSizes?.map((size: any) =>
    Number(size.price)
  );
  ...
  const { price: minPrice } = usePrice({
    amount: locale === 'vi' ? minP : (minP * 43) / 1000000,
    baseAmount: locale === 'vi' ? minP : (minP * 43) / 1000000,
    currencyCode: locale === 'vi' ? 'VND' : 'USD',
  });
  const { price: maxPrice } = usePrice({
    amount: locale === 'vi' ? maxP : (maxP * 43) / 1000000,
    baseAmount: locale === 'vi' ? maxP : (maxP * 43) / 1000000,
    currencyCode: locale === 'vi' ? 'VND' : 'USD',
  });
    ...
    >
      <div className="relative flex-shrink-0">
        <div className="flex overflow-hidden max-w-[230px] mx-auto transition duration-200 ease-in-out transform group-hover:scale-105 relative">
          <Image
            src={
              image.split('|').filter((image) => image !== '')[0] ??
              productPlaceholder
            }
            alt={dishName || 'Product Image'}
            width={230}
            height={200}
            quality={100}
        ...
        <div className="w-full h-full absolute top-0 pt-2.5 md:pt-3.5 px-3 md:px-4 lg:px-[18px] z-10 -mx-0.5 sm:-mx-1">
          ...
          <div className="inline-block product-count-button-position">
            {isAuthorized && <RenderPopupOrAddToCart data={product} />}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col px-3 md:px-4 lg:px-[18px] pb-5 lg:pb-6 lg:pt-1.5 h-full">
        ...
        <h2 className="text-skin-base text-13px sm:text-sm lg:text-15px leading-5 sm:leading-6 mb-1.5">
          {locale === 'vi' ? dishName : dishNameEn}
        </h2>
        <div className="text-13px sm:text-sm mt-auto">
          {locale === 'vi' ? type?.unit : type?.unitEn}
        </div>
      </div>
    </article>
  );
