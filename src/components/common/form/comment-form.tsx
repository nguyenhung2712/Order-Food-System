...
import { useCommentMutation } from '@framework/comment/use-comment';
...
const CommentForm: React.FC<CommentFormProps> = ({ blog, className = '' }) => {
  ...
  const { mutate: comment, isLoading } = useCommentMutation();
  ...
  function onSubmit(values: CommentFormValues) {
    comment({ ...values, image, blogId: blog?.id });
    setPreviewImage('');
    setValue('message', '');
    setIsHaving(false);
  }
