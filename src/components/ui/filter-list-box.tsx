...
export default function ListBox({ options }: { options: Option[] }) {
  ...
  const currentSelectedItem = sort_by
    ? options.find((o) => o.value === sort_by)!
    : options[0];
  ...
  function handleItemClick(values: Option) {
    ...
    if (locale === 'vi') {
      switch (values.value) {
        case 'highest':
          values.value = 'cao-nhat';
          break;
        case 'most-order':
          values.value = 'dat-nhieu-nhat';
          break;
        default:
          values.value = 'thap-nhat';
          break;
      }
    }
    router.push(
