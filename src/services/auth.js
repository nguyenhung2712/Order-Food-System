                ...
                .then(async (user) => {
                    ...
                    await Token.createToken(user, otpCode)
                        .then(async (token) => {
                            ...
                            await sendEmail(user.email, "Mã xác nhận", `Đây là mã OTP xác nhận tài khoản của bạn: ${otpCode}`);
                            resolve({
                                status: "success",
                                        ...
                                        <tr>
                                            <td style="text-align:center;padding: 50px 30px;">
                                                <img style="width:88px; margin-bottom:24px;"
                                                    src="https://cdn2.iconfinder.com/data/icons/default-1/100/.svg-6-512.png"
                                                    alt="Verified">
                                                <h2 style="font-size: 18px; color: #1ee0ac; font-weight: 400; margin-bottom: 8px;">
                                                    Your Email Verified.</h2>
                                                ...
                                                <div style="text-align:left">
                                                    ...
                                                    <p>
                                                        Hope you'll enjoy the experience, we're here if you have any questions, drop
                                                        us
...
const forgetPassword = ({ email }) => new Promise(async (resolve, reject) => {
    try {
        ...
        await Token.createToken(user, otpCode)
            .then(async (token) => {
                await sendEmail(email, "Mã xác nhận", `Đây là mã OTP xác nhận tài khoản của bạn: ${otpCode}`);
                resolve({
                    status: "success",
                    message: "Đã gửi mã OTP đến email của bạn."
                });
